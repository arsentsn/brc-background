...
export type MarbleBand = [number, number];

export interface MarbleProps {
  /** Colours of the flat bands, in order. Any length; five is the tuned default. */
  colors?: string[];
  /** Inclusive [start, end] texel range of each band in the ramp texture. */
  bands?: MarbleBand[];
  /** Zoom. Larger values zoom out into a denser field; smaller gives broader, calmer regions. */
  scale?: number;
  /** Morph cycles per second. The default is the studied shader's rate. */
  speed?: number;
  /** Constant offset into the ramp. It does not scroll. */
  offset?: number;
  /** Warp iterations. */
  iterations?: number;
  /** Multiplier over the base warp amplitude. */
  amplitude?: number;
  /** How the 2-D accumulator collapses to one scalar: the default is broad and calm, 1 gives much denser banding. */
  collapse?: number;
...
const DEFAULT_BANDS: MarbleBand[] = [
  [0, 29],
  [34, 45],
  [49, 77],
  [81, 109],
...
const BASE_AMPLITUDE: [number, number] = [11.08 / 467, 18.3 / 467];
const TRANSITION = 4;
...
const evenBands = (count: number): MarbleBand[] => {
  const width = RAMP_SIZE / count;
  return Array.from({ length: count }, (_, i): MarbleBand => [
...
function buildRamp(colors: string[], bands?: MarbleBand[]): Uint8Array {
...
void main() {
  ...
  // Tied to width, so the pattern keeps a constant on-screen density as the
...
export default function Marble({
  ...
  const settingsRef = useRef<MarbleSettings>({ scale, speed, offset, iterations, amplitude, collapse, paused });
